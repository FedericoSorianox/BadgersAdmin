...
router.get('/cash-detail', async (req, res) => {
    try {
        ...
        if (localDate) {
            // Force the parsing to use Uruguay Time (-03:00) so that 
            // a sale at 21:00 PM local yesterday (which is 00:00 UTC today) 
            // is NOT counted as today's sale.
            startOfDay = new Date(`${localDate}T00:00:00-03:00`);
            endOfDay = new Date(`${localDate}T23:59:59.999-03:00`);
        } else {
            startOfDay = new Date();
